...
input_file <- args[1]
# input_file <- "Simulations_10/var/jobs/Mutect_all/Sample98.mutect.g.vcf.gz"
# input_file <- "Simulations_3/var/jobs/Mutect_all/Sample0.mutect.g.vcf.gz"
vcf <- fread(input_file)

simulation <- sub("/.*", "", input_file)
sample <- sub(".*/Sample(\\d+)\\.mutect\\.g\\.vcf\\.gz", "Sample\\1", input_file)
...
# tp <- sum(final_result$Status=="TP") #198
# fp <- sum(final_result$Status=="FP") #74
# fn <- sum(final_result$Status=="FN") #19
...
# tp <- sum(final_result$Status=="TP") #210
# fp <- sum(final_result$Status=="FP") #62
# fn <- sum(final_result$Status=="FN") #7

output_file <- sub("mutect.g.vcf.gz", "evaluation.tab", input_file)
