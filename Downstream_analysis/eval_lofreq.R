...
## libraries 
suppressMessages(library(data.table)) #More efficient data handling
...
input_file <- args[1]
# input_file <- "Simulations_10/var/jobs/Lofreq_all/Sample98.output_indel.vcf"
# input_file <- "Simulations_3/var/jobs/Lofreq_all/Sample0.output.vcf"
# input_file <- "Simulations_3/var/jobs/Lofreq_all/Sample0.output_indel.vcf"
# input_file <- "Simulations_9/var/jobs/Lofreq_all/Sample12.output_indel.vcf"
# input_file <- "Simulations_41/var/jobs/Lofreq_all/Sample0.output_indel.vcf"
vcf <- fread(input_file)

#Get allele frequency
vcf$AF <- sub(".*AF=([^;]+).*", "\\1", vcf$INFO)
vcf <- vcf[,c("POS", "REF", "ALT", "AF")]

#Get the true variants for the sample and simulation:
simulation <- sub("/.*", "", input_file)
sample <- sub(".*/Sample(\\d+)\\.output_indel\\.vcf", "Sample\\1", input_file)
...
# tp <- sum(final_result$Status=="TP") #199, 133
# fp <- sum(final_result$Status=="FP") #125, 43
# fn <- sum(final_result$Status=="FN") #18, 342
...
# tp <- sum(final_result$Status=="TP") #211, 118
# fp <- sum(final_result$Status=="FP") #113, 38
# fn <- sum(final_result$Status=="FN") #6, 259

output_file <- sub("output_indel.vcf", "evaluation.tab", input_file)
